...
Tdbl = 0.15; % Double support phase period
...
for i = 1 : size(Feet,1)+1
    if(i==1) %--> switching from rest mode to walking mode
        ...
        %--> insert dbl support phase
        x_dbl = linspace(0,Feet(i,1),(Tdbl+delta_t)/delta_t).'; 
        y_dbl = linspace(0,Feet(i,2),(Tdbl+delta_t)/delta_t).'; 
        z_dbl = linspace(0,Feet(i,3),(Tdbl+delta_t)/delta_t).'; 
        ZMP_traj = [ZMP_traj; [x_dbl(2:end,:),y_dbl(2:end,:),...
            z_dbl(2:end,:)]]; 
        %--> end of insertion
    elseif(i>1 && i<=size(Feet,1)) %--> walking mode
        ...
        %--> insert dbl support phase
        if(i~=size(Feet,1))
            x_dbl = linspace(Feet(i-1,1),Feet(i,1),(Tdbl+delta_t)/delta_t).'; 
            y_dbl = linspace(Feet(i-1,2),Feet(i,2),(Tdbl+delta_t)/delta_t).'; 
            z_dbl = linspace(Feet(i-1,3),Feet(i,3),(Tdbl+delta_t)/delta_t).'; 
            ZMP_traj = [ZMP_traj; [x_dbl(2:end,:),y_dbl(2:end,:),...
                z_dbl(2:end,:)]]; 
        else
            x_dbl = linspace(Feet(i-1,1),(Feet(i,1)+Feet(i-1,1))/2,(Tdbl+delta_t)/delta_t).'; 
            y_dbl = linspace(Feet(i-1,2),(Feet(i,2)+Feet(i-1,2))/2,(Tdbl+delta_t)/delta_t).'; 
            z_dbl = linspace(Feet(i-1,3),Feet(i,3),(Tdbl+delta_t)/delta_t).'; 
            ZMP_traj = [ZMP_traj; [x_dbl(2:end,:),y_dbl(2:end,:),...
                z_dbl(2:end,:)]]; 
        end
...
for i = 1 : size(Sup_map,1)-1
    if(i==1)
        ...
        %--> insert in-between double support phase
        for j = 0:Tdbl/delta_t
            RF.traj = [RF.traj; Sup_map(RF.sup+1,1:3)];
            LF.traj = [LF.traj; Sup_map(LF.sup+1,1:3)];
            Sim_Sup = [Sim_Sup, "d"];
        end
    ...
    elseif(i>1 && i<=N) %--> Walk mode
        ...
        %--> insert in-between double support phase
        for j = 0:Tdbl/delta_t
            if(RF.sup) %--> if right foot was support
                RF.traj = [RF.traj; Sup_map(i,1:3)]; 
                LF.traj = [LF.traj; Sup_map(i+1,1:3)]; 
            else %--> if left foot is support
                RF.traj = [RF.traj; Sup_map(i+1,1:3)]; 
                LF.traj = [LF.traj; Sup_map(i,1:3)];
            end
            Sim_Sup = [Sim_Sup, "d"];
        end
        %--> end of insertion
...
% COM trajectory
COMx = wpg(delta_t,COMz_wpg,[0;0;0],size(ZMP_traj,1)*delta_t,ZMP_traj(:,1));
COMy = wpg(delta_t,COMz_wpg,[0;0;0],size(ZMP_traj,1)*delta_t,ZMP_traj(:,2));
COMz = [];
for i = 1 : size(Sup_map,1)-1
    if(Sup_map(i,1)==Sup_map(i+1,1))
        ...
        %--> insert double support phase
        if(i~=size(Sup_map,1)-1)
            for j = 0:Tdbl/delta_t
                COMz = [COMz, Sup_map(i,3)+COMz_wpg]; 
            end
        end
        %--> end of insertion
    else
        ...
        %--> insert double support phase
        for j = 0:Tdbl/delta_t
            COMz = [COMz, Sup_map(i+1,3)+COMz_wpg]; 
        end
        %--> end of insertion
    end
end
...
for i = 1 : size(Sup_map,1)-1
    if(i==1)
        ...
        if(i~=size(Sup_map,1)-1)
            for j = 0:Tdbl/delta_t
                Pel_orient = [Pel_orient, Orient_map(i+1)];
                if(R_sup)
                  Qr_yaw = [Qr_yaw, Orient_map(i+1)-Sup_map(i,4)]; %--> add increments to right yaw
                  Ql_yaw = [Ql_yaw, Sup_map(i+1,4)-Orient_map(i+1)]; %--> add increments to left yaw
                else
                  Ql_yaw = [Ql_yaw, Orient_map(i+1)-Sup_map(i,4)]; %--> add increments to left yaw
                  Qr_yaw = [Qr_yaw, Sup_map(i+1,4)-Orient_map(i+1)]; %--> add increments to right yaw
                end
            end
        end
        %--> end of insertion
