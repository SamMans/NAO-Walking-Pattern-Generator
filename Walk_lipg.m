...
Tsup = 1; % single support time

% Support feet stamps for gait
% Robot starts motion from a global frame that lies in the middle of feet
Feet = [0 0 0 0 1; %---> Start must have x=0 and orientation=0, other parameters are situation and workspace dependent
    0.05 0.1 0 0 2;   %---> Start must have abs(y-coordinate) = y_spacing --> standard beginning posture 
    0.1 0 0 0 3;
    0.15 0.1 0 0 4;
    0.2 0 0 0 5;
    0.25 0.1 0 0 6;
    0.3 0 0 0 7;               %----> Excluding initial and final double support phases (Feet,Orient)
    0.35 0.1 0 0 8;
    0.4 0 0 0 9;
    0.45 0.1 0 0 10;
    0.5 0 0 0 11;
    0.55 0.1 0 0 12; 
    0.55 0 0 0 13]; %--> Support feet info. (x,y,z,yaw orientation,timing in sec.)
...
Sup_map = [[Feet(1,1),0.1,Feet(1,3),0,0];Feet;...
    [Feet(N-1,1:4),Feet(N,5)+Feet(1,5)]]; %---> list of all support feet including both single and double


% COM trajectory
[COM_traj, Feet] = lipg(Feet, COMz_wpg, Tsup, [0,0], [0,0], delta_t, 0.5);
COMx = COM_traj(1,:); COMy = COM_traj(2,:);
COMz = [];
for i = 2 : size(Sup_map,1)-2
...
for i = 1 : size(Feet,1)-1
    for j = Feet(i,5):delta_t:(Feet(i+1,5)-delta_t)
        ZMP_traj = [ZMP_traj; [Feet(i,1),Feet(i,2),Feet(i,3)]];
    end
end
...
Sim_Sup = [];
SUP_arr = ["r","l"]; % Support array
if(Feet(1,2)<0)
    Sup_c = 1; % Support choice-right
else
    Sup_c = 2; % Support choice-left
end
for i = 2 : size(Sup_map,1)-2
    T = Sup_map(i+1,5)-Sup_map(i,5);
    for j = 0:delta_t:T-delta_t
        Sim_Sup = [Sim_Sup, SUP_arr(Sup_c)]; %--> fill support foot array
    end
    Sup_c = 2 - (Sup_c-1);
end
